...
export function ema(values: number[], period: number): number[] {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    else { const next = v * k + prev * (1 - k); out.push(next); prev = next; }
  }
  return out;
}
...
export function rsi(values: number[], period = 14): number[] {
  ...
  const avgGain = ema(gains, period); const avgLoss = ema(losses, period);
  const rsiArr: number[] = [50];
  for (let i = 0; i < avgGain.length; i++) {
    const ag = avgGain[i] || 0.00001; const al = avgLoss[i] || 0.00001;
    const rs = ag / al; rsiArr.push(100 - 100 / (1 + rs));
  }
...
export function pct(a?: number | null, b?: number | null) {
  if (a == null || b == null) return 0; return ((a - b) / b) * 100;
}
